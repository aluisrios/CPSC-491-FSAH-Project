<html>
<head>
    <link rel="stylesheet" href="CSS/Home.css">
</head>

<body>

    <div class="Banner">

        <div class="navbar">
            <img src="LOGO.png" class="logo">
            <ul>
                <li><a href="HomePage.php">Home</a></li>
                <li><a href="SignUp.php">Apply</a></li>
                <li><a href="supportPage.html">Support</a></li>
                <li><a href="LoginPage.php">Login</a></li>
            </ul>
        </div>

    <div class="content">
        <h1> Make applying for Financial Easy </h1>
        <p> FSAH is here to get all the Financial aid information in one place,
        <br>we can help with any question you make have, or how you could apply. </p>

        <div class="button">
            <a href='SignUp.php'><button type="button"><span></span>Apply</button></a>
            <a href='FAQ.html'><button type="button"><span></span>Info</button></a>
        </div>

    </div>


</body>
</html>